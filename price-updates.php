...
function generate_price_update_urls() {
    ...
    foreach ($post_ids as $post_id) {
        // Use the post's permalink instead of the Amazon link
...
// Add scraper API endpoint
add_action('rest_api_init', 'register_price_update_api');
function register_price_update_api() {
    register_rest_route('auto-workflows/v1', '/price-update-urls', array(
        'methods' => 'GET',
        'callback' => 'get_price_update_urls_api',
        'permission_callback' => '__return_true' // Public endpoint
    ));
    
    register_rest_route('auto-workflows/v1', '/update-price', array(
        'methods' => 'POST',
        'callback' => 'update_price_api',
        'permission_callback' => '__return_true' // Public endpoint
    ));
}
...
function price_updates_admin_css() {
    ?>
    <style>
        .price-decrease { color: #28a745; font-weight: bold; }
...
function track_pending_scraper_updates($post_ids) {
    ...
    // Remove updates older than 24 hours (they probably failed)
